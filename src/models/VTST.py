...
class VTST(nn.Module):
    def __init__(self, dataset="MNIST", latent_dim=128, num_classes=10, separate_body=False, beta=0.01, pretrained_qyx = None, accelerator="cpu", bound_qzx_var=False,
        ...
        self.qzx_model = construct_ClassYEncoder(dataset, self.latent_dim, simple_CNN=simple_CNN, num_layers=MLP_size, ViT_experiment=ViT_experiment)
        if self.bound_qzx_var:
            self.qzx_var = construct_EncoderVar(dataset, self.latent_dim, simple_CNN=simple_CNN, num_layers=MLP_size, ViT_experiment=ViT_experiment)
