...
class TST(nn.Module):
    def __init__(self, dataset="MNIST", latent_dim=128, num_classes=10, separate_body=False, pretrained_qyx = None, accelerator="cpu", MLP_size=3,
                 paper=None, simple_CNN=False, ViT_experiment=False, reinit_experiment=False, model_name_or_path='google/vit-base-patch16-224-in21k', ResNet50_experiment = False):
        super().__init__()
        ...
        if reinit_experiment:
            ...
            self.qzx_model = construct_ClassYEncoder(dataset, latent_dim, simple_CNN=simple_CNN, num_layers=MLP_size, ViT_experiment=ViT_experiment, ResNet50_experiment=ResNet50_experiment)
